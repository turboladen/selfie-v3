...
use selfie::progress_display::{ProgressManager, ProgressStyleType, ProgressDisplay};
use std::thread;
...
fn main() {
    ...
    for i in 0..=100 {
        ...
        if i % 20 == 0 {
            ...
            progress_manager.add_progress_line(
                "dep1-install",
                &format!("Processed {} of 100 steps", i),
            ).unwrap();
        }
    ...
    progress_manager.add_command_output(
        "pkg",
        "stdout",
        "Downloading example-pkg v1.0.0...",
    ).unwrap();
    thread::sleep(Duration::from_millis(800));
